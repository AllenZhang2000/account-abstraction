...
function hashToField(domain, msg, count) {
  ...
  for (let i = 0; i < count; i++) {
    const el = ethers.BigNumber.from(_msg.slice(i * u, (i + 1) * u)).mod(
      FIELD_ORDER
    );
    els.push(el);
  }
...
function expandMsg(domain, msg, outLen) {
  ...
  for (let i = 1; i < ell; i++) {
    ...
    const nb0 = ethers.zeroPad(ethers.utils.arrayify(b0), 32);
    const nbi = ethers.zeroPad(ethers.utils.arrayify(bi), 32);
    ...
    out.set(ethers.utils.arrayify(bi), 32 * (i - 1));
    bi = ethers.utils.sha256(ini);
  }

  out.set(ethers.utils.arrayify(bi), 32 * (ell - 1));
  return out;
}
...
function test_expand_msg() {
  for (let i = 0; i < vectors.length; i++) {
    ...
    assert.equal(ethers.utils.BigNumberhexlify(out), v.expected);
  }
}
